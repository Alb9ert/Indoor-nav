  ...
  // rules that ONLY apply to your TypeScript source files.
  {
    ...
    settings: {
      ...
      "import/resolver": {
        typescript: {
          alwaysTryTypes: true,
          // We poinpt it to all tsconfig files to ensure it finds all aliases.
    ...
    rules: {
      ...
      "@typescript-eslint/naming-convention": [
        ...
        // Allows `__foo` / `_foo` variables (e.g. globals, caches).
        {
          selector: "variable",
          format: ["camelCase", "UPPER_CASE", "PascalCase"],
          leadingUnderscore: "allow",
        },
        // Allows UPPER_CASE config keys (env schemas, etc.) and `__html`.
        {
          selector: "objectLiteralProperty",
          // Object keys often come from external contracts (env vars, HTTP, CSS maps, etc.).
          // We don't enforce a naming format here; other selectors still enforce conventions.
          format: null,
          leadingUnderscore: "allow",
        },
        // Allows route handler keys like `GET` / `POST`.
        {
